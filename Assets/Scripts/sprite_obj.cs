...
public class sprite_obj : MonoBehaviour {
	...
	void Start () {
        ...
        base_pos = new Vector3((float)(start.leftbound), (float)2, (float)(transform.position.z));
	...
	void Update () {
        if (!start.end && !start.pause && start.beta_testing)
        {
            ...
            {
                Vector3 base_pos = new Vector3((float)(start.leftbound), (float)2, (float)(transform.position.z));
            ...
            transform.position = new Vector3((float)(start.leftbound), (float)(2+ Random.Range(0,2)), (float)(transform.position.z));
        }
	}
    ...
    {
        start.end = true;
        GameObject.Find("test").GetComponent<Text>().text = other.tag;
    }
}
